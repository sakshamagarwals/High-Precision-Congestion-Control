    ...
    for line in f1:
        ...
        if (fct != -1): #could be -1 because of the weird way in which the file logs
            ...
            if(src/NUM_HOSTS_PER_AGG == dst/NUM_HOSTS_PER_AGG):
                ...
                ideal_data_time = ((flowsize*8.0)/link_bandwidth)/1e3 + 1*(((pktsize*8.0)/link_bandwidth)/1e3) + 2*(((pktsize*8.0)/(link_bandwidth))/1e3) + (4 * propagation_delay_in_ns * 1e-3)
                ideal_header_time = 2.0 * (8.0 * 40 / link_bandwidth)/1e3 + 2.0 * (8.0 * 40 / (link_bandwidth))/1e3  + (4 * propagation_delay_in_ns * 1e-3)
