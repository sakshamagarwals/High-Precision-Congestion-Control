...
static inline void print_trace(ns3::TraceFormat &tr){
	switch (tr.l3Prot){
		...
		case 0xFD: // NACK
			...
			break;
		case 0xFE: // PFC
			printf("%lu n:%u %u:%u %u %s ecn:%x %u %u %c %u %u %u %hu", tr.time, tr.node, tr.intf, tr.qidx, tr.qlen, EventToStr((ns3::Event)tr.event), tr.ecn, convert(tr.sip), convert(tr.dip), l3ProtToChar(tr.l3Prot), tr.pfc.time, tr.pfc.qlen, tr.pfc.qIndex, tr.size);
			break;
		case 0xFF: // CNP
			printf("%lu n:%u %u:%u %u %s ecn:%x %u %u %c %u %u %u %u %u", tr.time, tr.node, tr.intf, tr.qidx, tr.qlen, EventToStr((ns3::Event)tr.event), tr.ecn, convert(tr.sip), convert(tr.dip), l3ProtToChar(tr.l3Prot), tr.cnp.fid, tr.cnp.qIndex, tr.cnp.ecnBits, tr.cnp.seq, tr.size);
			break;
		case 0x0: // QpAv
			printf("%lu n:%u %u:%u %s %u %u %u %u", tr.time, tr.node, tr.intf, tr.qidx, EventToStr((ns3::Event)tr.event), convert(tr.sip), convert(tr.dip), tr.qp.sport, tr.qp.dport);
			break;
		default:
			printf("%lu n:%u %u:%u %u %s ecn:%x %u %u %x %u", tr.time, tr.node, tr.intf, tr.qidx, tr.qlen, EventToStr((ns3::Event)tr.event), tr.ecn, convert(tr.sip), convert(tr.dip), tr.l3Prot, tr.size);
			break;
	}
	printf("\n");
}
